...
| 頻度 | 毎週月曜 03:00 UTC (+ Actions の画面から手動実行) |
| 手順 | MPC から取得 → 抽出 → `tools/check_bodies.py` で検査 → 変化があれば commit |

MPC の元期 (Epoch) は半年ごとにしか動かないが、新しく見つかった天体と、観測が
積み増されたことによる軌道の更新はほぼ毎日入ってくる。週1回はそちらに追いつく
ための頻度。それでも待てないときは手動実行する。

`--full` を付けた実行では 180 MB の `mpcorb_extended.json.gz` を落として
155万件を走査する (Actions で3〜4分)。付けない場合、名前付き小惑星
(`named.json`) は更新されない。**決まった時刻の実行は毎回 `--full`**。
付けずに走らせると `popular.json` から名前付き小惑星が抜け落ち (64件 → 38件)、
...
> 直接デプロイする形 (`actions/deploy-pages`) に変えてもよい。中身が動いた回は
> 1回あたり約 8 MB (圧縮後 2.5 MB 程度) が積み上がる。
