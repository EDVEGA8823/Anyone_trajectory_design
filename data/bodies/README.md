...
- MPC のデータは自由に利用できるが、出典を明記すること。生成したファイルにも
...
| `popular.json` | よく使う天体 (`tools/popular_bodies.txt` で指定) | 61 | 4 KB |
...
```
番号があれば   a:162173   c:67P と同じ番号体系ではないので接頭辞で分ける
無ければ符号   a:1998 KY26   c:C/1995 O1
```

小惑星は `a:`、彗星は `c:`。**小惑星番号と周期彗星番号は別の体系** (1 は Ceres
だが 1P は Halley) なので、接頭辞を落とさないこと。
...
```
a_km = a * AU
n    = sqrt(mu_sun / a_km^3)          [rad/s]
M(t) = M0 + n * (t - epoch) * 86400   [rad]
E    = solve_kepler(e, M(t))
```

彗星 (e < 1):

```
a = q / (1 - e)          として上と同じ。M は元期ではなく近日点通過 tp から測る
M(t) = n * (t - tp) * 86400
```

## 未対応

- **放物線・双曲線の彗星** (`e ≧ 1`、934件中115件)。位置の計算 (`get_planets_pos_E`)
  は双曲線に対応しているが、ケプラー方程式を解く `solve_kepler` が楕円専用なので、
  双曲線用の解法を足すまでは扱えない。天体を選ぶ画面では伏せておく
